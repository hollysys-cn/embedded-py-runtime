...
- Q: 演示程序的目录组织结构应该如何设计？ → A: 更新现有的 `examples/plcopen/` 目录结构，填充已有的占位程序
- Q: 演示程序之间是否应共享辅助模块？ → A: 共享，创建 `examples/plcopen/common/` 目录存放公共辅助代码
- Q: 演示程序的数据输出格式应该采用什么标准？ → A: CSV 格式，逗号分隔，首行为列名
- Q: 现有占位示例如何与规格中的用户故事对应？ → A: 扩展目录结构，新增 `ramp_limit_demo` 和 `integrator_demo`，共 5 个演示程序
- Q: 演示程序的仿真运行时长和输出频率如何配置？ → A: 编译时配置，通过头文件中的宏定义设置
...
3. **Given** PID 控制器处于自动模式，**When** 在第 150 周期切换到手动模式（输出锁定 50%），在第 170 周期切回自动模式，**Then** 输出应平滑过渡，无阶跃跳变
...
### User Story 4 - 积分/微分演示 (Priority: P3)

作为计量系统和振动监测开发者，我需要一个展示积分器和微分器功能的演示程序，模拟流量累计和变化率检测。

**Why this priority**: 积分器用于生产统计、计费等场景；微分器用于速度/加速度计算。两者是工业测量的基础功能，共享同一演示程序可简化结构。

**Independent Test**: 输入恒定流量信号验证积分累计；输入斜坡位置信号验证微分输出恒定速度。
...
4. **Given** 微分器配置采样周期 0.01s，**When** 输入斜率 100/s 的斜坡信号，**Then** 输出应稳定在 100（±5% 误差）
5. **Given** 微分器配置了滤波时间常数 0.05s，**When** 输入阶跃信号，**Then** 输出应为平滑脉冲而非无穷大尖峰

---

### User Story 5 - 综合系统演示 (Priority: P2)

作为系统集成开发者，我需要一个展示多功能块协同工作的综合演示程序，模拟完整的控制系统（设定值斜坡 + PID 跟踪 + 滤波 + 限幅）。

**Why this priority**: 综合演示展示功能块库的完整应用模式，是用户构建实际控制系统的最佳参考。

**Independent Test**: 运行程序后验证设定值按斜坡变化、PID 跟踪斜坡设定值、输出被限幅在安全范围内。

**Acceptance Scenarios**:

1. **Given** RAMP 配置上升速率为 5 单位/秒，目标从 25 变为 80，**When** 运行控制循环，**Then** 设定值应在约 11 秒内平滑上升到 80
2. **Given** PID 控制器跟踪斜坡设定值，**When** 设定值持续变化，**Then** 过程值应以最小延迟跟踪设定值
3. **Given** LIMIT 配置输出范围 [0, 100]，**When** PID 输出超出范围，**Then** 输出应被限制并触发限幅标志
...
- **参数越界保护**: 用户输入非法参数（如负的时间常数）时，初始化函数应返回错误码（-1）并通过 stderr 输出警告信息，同时使用安全默认值
...
- **FR-008**: 系统 MUST 提供积分器/微分器综合演示程序，展示累计量计算和变化率检测

**综合演示**:

- **FR-009**: 系统 MUST 提供综合系统演示程序，展示 RAMP + PID + PT1 + LIMIT 协同工作
...
- **FR-022**: 演示程序目录结构 MUST 包含 5 个独立程序：`pid_control_demo`、`filter_demo`、`full_system_demo`、`ramp_limit_demo`、`integrator_demo`
- **FR-023**: 系统 MUST 在 `examples/plcopen/common/` 目录提供共享辅助模块（被控对象模型、噪声发生器等）
- **FR-024**: 演示程序输出 MUST 采用 CSV 格式（UTF-8 无 BOM 编码），首行为列名，数据逗号分隔
- **FR-025**: 仿真参数（运行时长、采样周期、输出频率）MUST 通过头文件宏定义配置
...
- **SC-006**: 演示程序输出 CSV 格式数据，可直接导入 Excel 或数据分析工具生成控制曲线图
...
4. **输出格式**: 演示程序输出 CSV 格式，包含时间戳、设定值、过程值、控制输出等列，首行为列名
...
9. **目录结构**: 演示程序位于 `examples/plcopen/` 目录下，共享代码位于 `examples/plcopen/common/`
10. **配置方式**: 仿真参数通过 `sim_config.h` 头文件中的宏定义配置，支持编译时定制
