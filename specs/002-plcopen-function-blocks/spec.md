...
1. **Given** PID 功能块已初始化（Kp=1.0, Ki=0.1, Kd=0.05），**When** 设定值从 0 跳变到 100，**Then** 输出应在 50 个采样周期内收敛到设定值（95% 达到稳态），稳态误差小于 1%
...
- **FR-002**: PID 功能块 MUST 支持积分抗饱和（Anti-Windup）机制，采用条件积分法：当输出达到上限时停止正向积分累加，当输出达到下限时停止负向积分累加，确保积分项不继续增大饱和程度
- **FR-003**: PID 功能块 MUST 支持手动/自动模式无扰动切换
- **FR-004**: PID 功能块 MUST 支持输出限幅配置（可配置上限和下限）
- **FR-005**: PID 功能块的微分项 MUST 采用微分项先行滤波（Derivative on Measurement），即仅对过程测量值求微分而非对误差求微分，以避免设定值突变产生的微分尖峰
- **FR-006**: 系统 MUST 提供一阶惯性滤波器（PT1）功能块
- **FR-007**: 系统 MUST 提供斜坡发生器（RAMP）功能块，支持独立的上升/下降速率
- **FR-008**: 系统 MUST 提供限幅器（LIMIT）功能块，并输出限幅状态

**辅助功能块**:

- **FR-009**: 系统 MUST 提供死区处理（DEADBAND）功能块
- **FR-010**: 系统 MUST 提供积分器（INTEGRATOR）功能块，支持复位和限幅
- **FR-011**: 系统 MUST 提供微分器（DERIVATIVE）功能块，内置滤波以避免噪声放大

**通用要求**:

- **FR-012**: 所有功能块 MUST 使用 C11 标准实现
- **FR-013**: 所有功能块 MUST 可在 ARM Cortex-M4 平台上运行
- **FR-014**: 所有功能块 MUST 提供初始化和周期执行两个独立函数（如 `FB_PID_Init()` 和 `FB_PID_Execute()`）
- **FR-015**: 所有功能块首次执行时 MUST 使用首次输入值作为初始输出或内部状态，实现无跳变平滑启动
- **FR-016**: 所有功能块 MUST 支持多实例（每个实例独立状态）
- **FR-017**: 所有功能块 MUST 包含数值保护（溢出、除零、NaN检测）。溢出时输出限幅到有效范围（正溢出限幅到 FLT_MAX，负溢出限幅到 -FLT_MAX）并设置错误标志；除法运算中的关键参数（时间常数 τ (time_constant)、采样周期 Ts (sample_time) 等，单位：秒）小于 1e-6 时使用 1e-6 替代，避免除零；检测到 NaN/Inf 输入时输出零并设置错误标志
- **FR-018**: 所有功能块 MUST 提供状态输出（正常、限幅、错误等），通过状态码枚举类型返回运行状态
- **FR-019**: 所有源代码注释 MUST 使用简体中文（符合项目 Constitution 要求），函数和变量命名遵循 PLCopen 英文约定（行业标准）
- **FR-020**: [预留 v2.0] 功能块调试和诊断接口（未来扩展，不包含在当前版本范围）

**接口规范**:

- **FR-021**: 功能块接口 MUST 遵循 PLCopen 命名约定
- **FR-022**: 功能块 MUST 使用结构体封装所有参数和状态，采用 `<FB名>_t` 命名（如 `FB_PID_t`）
- **FR-023**: 功能块初始化函数 MUST 验证配置参数有效性（采样周期 Ts > 0 且 < 1000s，时间常数 τ > 0，限幅上限 > 下限等），无效配置返回错误码并阻止功能块启动。验证逻辑在 Init 函数内实现，无需独立的 ValidateConfig 函数

### Key Entities

- **FB_PID**: PID 控制器 - 包含 Kp、Ki、Kd 参数，手动/自动模式，积分限幅，输出限幅，微分项先行滤波（仅对测量值求微分）
- **FB_PT1**: 一阶惯性滤波器 - 包含时间常数 τ (time_constant)，采样周期 Ts (sample_time)，内部状态
...
- **SC-002**: 所有功能块单次执行时间 < 10μs（测量条件：Cortex-M4 @168MHz，编译优化 -O2，使用 DWT 周期计数器测量纯函数执行时间，不含中断和上下文切换开销）
...
7. **测试框架**: 单元测试使用 Unity 嵌入式 C 测试框架，支持目标硬件上的测试执行（通过串口输出或半主机模式）
8. **参考测试系统**: SC-001 中的"典型一阶系统"指传递函数 G(s) = 1/(τs+1)，其中增益 K=1.0，时间常数 τ=1.0s，用于 PID 控制器性能验证
