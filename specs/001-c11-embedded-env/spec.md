...
3. **Given** 项目有外部依赖（通过 CMakeLists.txt 中的 `link_libraries` 或手动配置的库路径），**When** 执行构建，**Then** 正确链接所需的依赖库并生成可执行文件（注：不包括自动下载依赖，假设依赖已预先配置）
...
9. **外部依赖自动管理**: 不包括自动下载、安装或管理第三方库的依赖管理系统（如 Conan、vcpkg）。项目依赖假设通过手动配置 CMake 的 `find_package()`、Git submodules 或供应商 SDK 提供。未来可作为增强功能支持
